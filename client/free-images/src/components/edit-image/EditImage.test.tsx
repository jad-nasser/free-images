...
const server = setupServer(
  ...
  rest.patch("/images/update-image", (req, res, ctx) => res(ctx.status(200))),
  rest.delete("/images/delete-image", (req, res, ctx) => res(ctx.status(200)))
);
...
describe("Testing EditImage component", () => {
  ...
  test('Testing when deleting the image it should shows "Image successfully deleted"', async () => {
    ...
    user.click(await screen.findByRole("button", { name: "Delete Image" }));
  ...
  test("Testing when the server sends an error it should shows the error", async () => {
    ...
    server.use(
      rest.patch("/images/update-image", (req, res, ctx) =>
    ...
    user.click(screen.getByRole("button", { name: "Edit Image" }));
